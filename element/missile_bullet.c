...
void Missile_bullet_update(Elements *self)
{
    ...
    if(Obj->direction == 0 && Obj->x < (WIDTH - BOARD_W)){
        self->dele = true;
        chara->bomb_cnt--;
    }
    if(Obj->direction == 1 && Obj->x > (WIDTH - ONE_GRID)){
        self->dele = true;
        chara->bomb_cnt--;
    }
    if(Obj->direction == 2 && Obj->y < 0){
        self->dele = true;
        chara->bomb_cnt--;
    }
    if(Obj->direction == 3 && Obj->y > (BOARD_H - ONE_GRID)){
        self->dele = true;
        chara->bomb_cnt--;
    }
...
void Missile_bullet_interact(Elements *self, Elements *tar)
{
    Missile_bullet *Obj = ((Missile_bullet *)(self->pDerivedObj));
    Character *chara = ((Character *)(Obj->player->pDerivedObj));
    Elements *player = ((Elements *)(Obj->player));
    if (tar->label == House_L)
    {
        House *house = ((House *)(tar->pDerivedObj));
        if (house->hitbox->overlap(house->hitbox, Obj->hitbox))
        {
            self->dele = true;
            chara->bomb_cnt--;
        }
    }
    // else if (tar->label == Character_L && player->label != Character_L)
    // {
    //     Character *cha = ((Character *)(tar->pDerivedObj));
    //     if (cha->hitbox->overlap(cha->hitbox, Obj->hitbox))
    //     {
    //         self->dele = true;
    //         chara->bomb_cnt--;
    //     }
    // }
    // else if (tar->label == Character1_L && player->label != Character1_L)
    // {
    //     Character *cha = ((Character *)(tar->pDerivedObj));
    //     if (cha->hitbox->overlap(cha->hitbox, Obj->hitbox))
    //     {
...
void Missile_bullet_draw(Elements *self)
{
    Missile_bullet *Obj = ((Missile_bullet *)(self->pDerivedObj));
    if(Obj->direction == 0)
    al_draw_bitmap(Obj->img3, Obj->x, Obj->y, 0);
    if(Obj->direction == 1)
